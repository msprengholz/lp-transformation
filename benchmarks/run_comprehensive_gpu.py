#!/usr/bin/env python3
"""
Comprehensive benchmark with optional GPU acceleration.

Strategy:
- If slangpy (GPU) is available: Sobol starts → GPU batch LP filter → GPU iRprop
- Falls back to CPU (numba) if slangpy is not available
"""
import sys, os, time
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

import numpy as np
from src.lp_functions import _z2_z3, _norm_factors, compute_lp_rmse
from src.test_cases import LP_VIQUERAT, LP_SPRENGHOLZ_48, _data_dir


# ──────────────────────────────────────────────
# Starting point generators
# ──────────────────────────────────────────────

try:
    from scipy.stats.qmc import Sobol
    HAS_SOBOL = True
except ImportError:
    HAS_SOBOL = False


class StartGenerator:
    """Generate space-filling starting points in [-pi/2, pi/2]."""
    def __init__(self, dim, seed=42):
        self.dim = dim
        self.seed = seed
        self.count = 0
        if HAS_SOBOL:
            self.sobol = Sobol(d=dim, scramble=True, seed=seed)
        else:
            self.rng = np.random.default_rng(seed)
        self._cache = []

    def __iter__(self):
        return self

    def __next__(self):
        self.count += 1
        if self.count <= len(self._cache):
            return self._cache[self.count - 1]
        if HAS_SOBOL:
            pts = self.sobol.random()
            lam = (pts * np.pi - np.pi / 2).astype(np.float32)
        else:
            lam = (self.rng.random(self.dim, dtype=np.float32) * np.pi - np.pi / 2)
        if len(self._cache) < 100000:
            self._cache.append(lam)
        return lam


# ──────────────────────────────────────────────
# Solution comparison
# ──────────────────────────────────────────────

def _round_key(lam_rad, decimals=1):
    deg = np.rad2deg(lam_rad)
    deg = (deg + 90) % 180 - 90
    return tuple(np.round(deg, decimals))


def _load_known_solutions(csv_name):
    path = _data_dir() / csv_name
    data = np.loadtxt(path, delimiter=";", dtype=np.float32)
    angles_deg = data[:, 1:-1]
    sols = set()
    for row in angles_deg:
        row = (row + 90) % 180 - 90
        sols.add(tuple(np.round(row, decimals)))
    return sols


# ──────────────────────────────────────────────
# GPU-accelerated discovery
# ──────────────────────────────────────────────

def _init_gpu():
    """Try to initialize GPU (SlangPy). Returns (dev, mod_lp, mod_irprop) or None."""
    try:
        import slangpy as sl
        from slangpy import grid, Tensor
        from src.gpu_lp import create_gpu_lp_solver, batch_lp_gpu
        from src.gpu_irprop import SLANG_IRPROP_STEP2, gpu_batch_irprop
        
        N = 12
        Z2, Z3 = _z2_z3(N)
        dev, mod_lp = create_gpu_lp_solver(N, Z2, Z3)
        mod_irprop = sl.Module.load_from_source(dev, "irprop_step2_12", SLANG_IRPROP_STEP2)
        
        # Warmup
        warmup = np.random.random((100, N)).astype(np.float32) * np.pi - np.pi / 2
        _ = batch_lp_gpu(dev, mod_lp, warmup, N)
        _ = gpu_batch_irprop(dev, mod_irprop, warmup, LP_VIQUERAT, max_iter=10)
        
        return dev, mod_lp, mod_irprop
    except Exception as e:
        print(f"  GPU init failed: {e}", flush=True)
        return None


def benchmark_viquerat_gpu(dev, mod_lp, mod_irprop, max_starts=50000, top_k=1500, irprop_iters=100):
    """GPU-accelerated Viquerat discovery: Sobol → GPU LP filter → GPU iRprop."""
    from src.gpu_lp import batch_lp_gpu
    from src.gpu_irprop import gpu_batch_irprop
    
    known = _load_known_solutions("viquerat_12_layer_solutions_complete.csv")
    target_count = len(known)
    found = set()
    target = LP_VIQUERAT
    N = 12
    
    from scipy.stats.qmc import Sobol
    sampler = Sobol(d=N, scramble=True, seed=42)
    
    t_start = time.perf_counter()
    
    # Phase 1: Generate Sobol starts and evaluate on GPU
    batch_size = 10000
    all_starts = []
    all_losses = []
    starts_used = 0
    
    while starts_used < max_starts:
        remaining = min(batch_size, max_starts - starts_used)
        new_starts = sampler.random(remaining).astype(np.float32) * np.pi - np.pi / 2
        starts_used += remaining
        
        # GPU batch LP
        lps = batch_lp_gpu(dev, mod_lp, new_starts, N)
        losses = np.sum((lps - target) ** 2, axis=1)
        all_starts.append(new_starts)
        all_losses.append(losses)
    
    all_starts = np.vstack(all_starts)
    all_losses = np.concatenate(all_losses)
    
    # Phase 2: Pick top-K and run GPU iRprop
    top_k_actual = min(top_k, len(all_losses))
    top_indices = np.argsort(all_losses)[:top_k_actual]
    best_starts = all_starts[top_indices]
    
    best_angles, best_losses = gpu_batch_irprop(dev, mod_irprop, best_starts, target, max_iter=irprop_iters)
    
    # Collect solutions
    for i in range(top_k_actual):
        if best_losses[i] < 0.05:  # Loss threshold (LP space, not RMSE)
            key = _round_key(best_angles[i])
            if key not in found:
                found.add(key)
                if len(found) >= target_count:
                    elapsed = time.perf_counter() - t_start
                    return elapsed, starts_used, len(found)
    
    elapsed = time.perf_counter() - t_start
    return elapsed, starts_used, len(found)


def benchmark_viquerat_cpu(max_starts=50000):
    """CPU-based Viquerat discovery using numba solver."""
    try:
        from src.numba_solver import optimize_laminate_numba as solver
        SOLVER_NAME = "numba"
    except ImportError:
        from src.numpy_solver import optimize_laminate as solver
        SOLVER_NAME = "numpy"
    
    known = _load_known_solutions("viquerat_12_layer_solutions_complete.csv")
    target_count = len(known)
    found = set()
    gen = StartGenerator(12)
    t_start = time.perf_counter()
    
    for attempt in range(1, max_starts + 1):
        lam = next(gen)
        opt, losses = solver(lam.reshape(1, -1), LP_VIQUERAT)
        best_loss = float(losses[0])
        
        if best_loss < 1e-3:
            key = _round_key(opt[0], 1)
            if key not in found:
                found.add(key)
                if len(found) >= target_count:
                    elapsed = time.perf_counter() - t_start
                    return elapsed, attempt, len(found)
        
        if attempt % 500 == 0:
            print(f"  CPU: {len(found)}/{target_count} after {attempt} starts "
                  f"({time.perf_counter()-t_start:.1f}s)", flush=True)
    
    elapsed = time.perf_counter() - t_start
    return elapsed, max_starts, len(found)


def benchmark_sprengholz_gpu(dev, mod_lp, mod_irprop, time_limit=60.0, batch_size=500):
    """GPU-accelerated 48-layer discovery."""
    from src.gpu_lp import create_gpu_lp_solver, batch_lp_gpu
    from src.gpu_irprop import gpu_batch_irprop
    
    N48 = 48
    Z2_48, Z3_48 = _z2_z3(N48)
    target = LP_SPRENGHOLZ_48
    
    # Create 48-layer GPU solver
    dev48, mod_lp48 = create_gpu_lp_solver(N48, Z2_48, Z3_48)
    from src.gpu_48layer import SLANG_IRPROP_48
    import slangpy as sl
    mod_irprop48 = sl.Module.load_from_source(dev48, "irprop_48", SLANG_IRPROP_48)
    
    found = set()
    from scipy.stats.qmc import Sobol
    sampler = Sobol(d=N48, scramble=True, seed=42)
    t_end = time.perf_counter() + time_limit
    starts_total = 0
    
    while time.perf_counter() < t_end:
        # Generate batch
        starts = sampler.random(batch_size).astype(np.float32) * np.pi - np.pi / 2
        starts_total += batch_size
        
        # GPU LP filter
        lps = batch_lp_gpu(dev48, mod_lp48, starts, N48)
        losses = np.sum((lps - target) ** 2, axis=1)
        
        # Top-K filter
        top_k = min(500, batch_size)
        top_indices = np.argsort(losses)[:top_k]
        best_starts = starts[top_indices]
        
        # GPU iRprop
        best_angles, best_losses = gpu_batch_irprop(dev48, mod_irprop48, best_starts, target, max_iter=100)
        
        for i in range(top_k):
            if best_losses[i] < 0.5:
                key = _round_key(best_angles[i])
                found.add(key)
    
    print(f"  48-layer GPU: {len(found)} unique solutions in {starts_total} starts", flush=True)
    return len(found), starts_total


# ──────────────────────────────────────────────
# Run
# ──────────────────────────────────────────────

def run():
    print("Comprehensive benchmark [GPU-accelerated]", flush=True)
    print("=" * 70, flush=True)
    
    # Try GPU first
    gpu = _init_gpu()
    
    if gpu is not None:
        dev, mod_lp, mod_irprop = gpu
        print(f"GPU: Available (SlangPy)", flush=True)
        
        print("\n--- Viquerat 12-layer discovery (GPU) ---", flush=True)
        
        # Parameter sweep
        results = []
        for max_starts, top_k, iters in [
            (10000, 1000, 50),
            (10000, 1500, 100),
            (20000, 1500, 50),
            (20000, 1500, 100),
            (20000, 2000, 100),
            (50000, 3000, 100),
            (50000, 1500, 50),
        ]:
            t, starts, found = benchmark_viquerat_gpu(
                dev, mod_lp, mod_irprop, max_starts=max_starts, 
                top_k=top_k, irprop_iters=iters)
            print(f"  starts={max_starts:>6d}, top_k={top_k:>5d}, iters={iters:>3d}: "
                  f"{t:.2f}s, {found}/112", flush=True)
            results.append((max_starts, top_k, iters, t, found))
        
        # Find best config that finds all solutions
        complete = [(r[0], r[1], r[2], r[3], r[4]) for r in results if r[4] >= 112]
        if complete:
            best = min(complete, key=lambda r: r[3])
            print(f"\nBest complete config: starts={best[0]}, top_k={best[1]}, "
                  f"iters={best[2]}, time={best[3]:.3f}s", flush=True)
            vq_time = best[3]
            vq_starts = best[0]
            vq_found = best[4]
        else:
            # Fall back to best found count
            best = max(results, key=lambda r: r[4])
            print(f"\nNo config found all 112 solutions. Best: {best[4]}/112", flush=True)
            vq_time = best[3]
            vq_starts = best[0]
            vq_found = best[4]
    else:
        print("GPU: Not available, using CPU", flush=True)
        vq_time, vq_starts, vq_found = benchmark_viquerat_cpu()
    
    # 48-layer Sprengholz throughput (always CPU for now)
    print("\n--- Sprengholz 48-layer throughput (CPU) ---", flush=True)
    try:
        from src.numba_solver import optimize_laminate_numba as solver
    except ImportError:
        from src.numpy_solver import optimize_laminate as solver
    
    found_48 = set()
    gen = StartGenerator(48)
    t_end = time.perf_counter() + 60.0
    completed = 0
    
    for attempt in range(1, 30000 + 1):
        if time.perf_counter() > t_end:
            break
        lam = next(gen)
        opt, losses = solver(lam.reshape(1, -1), LP_SPRENGHOLZ_48,
                              n_coarse_fine=2, irprop_grad_tol=1e-4)
        completed += 1
        if float(losses[0]) < 2e-2:
            key = _round_key(opt[0], 1)
            found_48.add(key)
    
    print(f"  48-layer: {len(found_48)} unique solutions in {completed} starts (60s)", flush=True)
    
    print()
    print("METRIC viquerat_discovery_time=%.3f" % vq_time, flush=True)
    print("METRIC viquerat_starts=%d" % vq_starts, flush=True)
    print("METRIC viquerat_found=%d" % vq_found, flush=True)
    print("METRIC sprengholz_solutions=%d" % len(found_48), flush=True)
    print("METRIC sprengholz_starts=%d" % completed, flush=True)


if __name__ == "__main__":
    run()