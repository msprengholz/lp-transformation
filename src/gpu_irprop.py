#!/usr/bin/env python3
"""
GPU iRprop: Batch iRprop on GPU using SlangPy.

Key insight: irprop_step returns float (loss per start), which SlangPy
can batch-return as numpy array. Angles/steps/grads are updated via
in-out device buffers, read back after each iteration.
"""
import sys, os, time
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

import numpy as np
import slangpy as sl
from slangpy import grid, Tensor

from src.lp_functions import _z2_z3, _norm_factors, compute_lp_rmse
from src.test_cases import LP_VIQUERAT

N = 12
Z2, Z3 = _z2_z3(N)
invN, N2, N3 = _norm_factors(N)

z2_str = ', '.join(str(int(round(z))) for z in Z2)
z3_str = ', '.join(str(int(round(z))) for z in Z3)

# iRprop step kernel that RETURNS the loss per start
SLANG_IRPROP_STEP = f"""
float irprop_step(int call_id,
                  float* angles,     // M * N  (read-write)
                  float* step_sizes,  // M * N  (read-write)
                  float* prev_grad,  // M * N  (read-write)
                  float* target_lp,  // 12     (read-only)
                  int M) {{
    int m = call_id;
    if (m >= M) return 0.0;
    
    float invN_val = 1.0 / {N}.0;
    float N2_val = 2.0 / ({N}.0 * {N}.0);
    float N3_val = 4.0 / ({N}.0 * {N}.0 * {N}.0);
    float z2[{N}] = {{{z2_str}}};
    float z3[{N}] = {{{z3_str}}};
    float n_p = 1.2;
    float n_m = 0.5;
    float s_max = 0.5;
    float s_min_val = 0.00001;
    
    // Compute trig
    float cos2[{N}], sin2a[{N}], cos4[{N}], sin4a[{N}];
    float c2s = 0.0, s2s = 0.0, c4s = 0.0, s4s = 0.0;
    float dc2z2 = 0.0, ds2z2 = 0.0, dc4z2 = 0.0, ds4z2 = 0.0;
    float dc2z3 = 0.0, ds2z3 = 0.0, dc4z3 = 0.0, ds4z3 = 0.0;
    
    for (int i = 0; i < {N}; i++) {{
        float lam = angles[m * {N} + i];
        cos2[i] = cos(lam * 2.0); sin2a[i] = sin(lam * 2.0);
        cos4[i] = cos(lam * 4.0); sin4a[i] = sin(lam * 4.0);
        c2s += cos2[i]; s2s += sin2a[i]; c4s += cos4[i]; s4s += sin4a[i];
        float z2i = z2[i]; float z3i = z3[i];
        dc2z2 += cos2[i] * z2i; ds2z2 += sin2a[i] * z2i;
        dc4z2 += cos4[i] * z2i; ds4z2 += sin4a[i] * z2i;
        dc2z3 += cos2[i] * z3i; ds2z3 += sin2a[i] * z3i;
        dc4z3 += cos4[i] * z3i; ds4z3 += sin4a[i] * z3i;
    }}
    
    // Compute LP
    float lp[12];
    lp[0] = c2s * invN_val;  lp[1] = s2s * invN_val;
    lp[2] = c4s * invN_val;  lp[3] = s4s * invN_val;
    lp[4] = dc2z2 * N2_val;  lp[5] = ds2z2 * N2_val;
    lp[6] = dc4z2 * N2_val;  lp[7] = ds4z2 * N2_val;
    lp[8] = dc2z3 * N3_val;  lp[9] = ds2z3 * N3_val;
    lp[10] = dc4z3 * N3_val; lp[11] = ds4z3 * N3_val;
    
    // Compute loss
    float loss = 0.0;
    for (int j = 0; j < 12; j++) {{
        float diff = target_lp[j] - lp[j];
        loss += diff * diff;
    }}
    
    // iRprop step for each layer
    for (int k = 0; k < {N}; k++) {{
        float grad_k = -2.0 * (
            -2.0 * sin2a[k] * (target_lp[0] - lp[0]) +
             2.0 * cos2[k] * (target_lp[1] - lp[1]) +
            -2.0 * sin4a[k] * (target_lp[2] - lp[2]) +
             2.0 * cos4[k] * (target_lp[3] - lp[3]) +
            -2.0 * sin2a[k] * z2[k] * (target_lp[4] - lp[4]) +
             2.0 * cos2[k] * z2[k] * (target_lp[5] - lp[5]) +
            -2.0 * sin4a[k] * z2[k] * (target_lp[6] - lp[6]) +
             2.0 * cos4[k] * z2[k] * (target_lp[7] - lp[7]) +
            -2.0 * sin2a[k] * z3[k] * (target_lp[8] - lp[8]) +
             2.0 * cos2[k] * z3[k] * (target_lp[9] - lp[9]) +
            -2.0 * sin4a[k] * z3[k] * (target_lp[10] - lp[10]) +
             2.0 * cos4[k] * z3[k] * (target_lp[11] - lp[11])
        );
        grad_k *= -2.0;
        
        float ss = step_sizes[m * {N} + k];
        float pg = prev_grad[m * {N} + k];
        
        float new_ss = ss;
        float new_pg = grad_k;
        
        if (pg * grad_k > 0.0) {{
            new_ss = min(ss * n_p, s_max);
        }} else if (pg * grad_k < 0.0) {{
            new_ss = max(ss * n_m, s_min_val);
            new_pg = 0.0;
        }}
        
        float new_angle = angles[m * {N} + k] - sign(new_pg) * new_ss;
        new_angle = max(-1.57079632679, min(1.57079632679, new_angle));
        
        angles[m * {N} + k] = new_angle;
        step_sizes[m * {N} + k] = new_ss;
        prev_grad[m * {N} + k] = new_pg;
    }}
    
    return loss;
}}
"""


def gpu_batch_irprop(dev, mod, lams_init, target_lp, max_iter=100):
    """Run batch iRprop on GPU. All M starts processed in parallel."""
    M, N_layers = lams_init.shape
    
    # State arrays - kept on GPU between iterations
    angles = lams_init.flatten().astype(np.float32).copy()
    step_sizes = np.full(M * N_layers, 0.01, dtype=np.float32)
    prev_grad = np.zeros(M * N_layers, dtype=np.float32)
    target = target_lp.astype(np.float32)
    
    best_angles = angles.copy()
    best_losses = np.full(M, 1e10, dtype=np.float32)
    
    for iteration in range(max_iter):
        # Create tensors for inputs/outputs
        a_t = Tensor.from_numpy(dev, angles)
        s_t = Tensor.from_numpy(dev, step_sizes)
        g_t = Tensor.from_numpy(dev, prev_grad)
        tgt_t = Tensor.from_numpy(dev, target)
        
        # Dispatch iRprop step - returns loss per start
        losses = mod.irprop_step(
            grid(shape=(M,)),
            a_t.storage.device_address,
            s_t.storage.device_address,
            g_t.storage.device_address,
            tgt_t.storage.device_address,
            int(M),
            _result="numpy"
        )
        
        # Read back updated state from GPU
        angles = a_t.storage.to_numpy().view(np.float32)[:M * N_layers].copy()
        step_sizes = s_t.storage.to_numpy().view(np.float32)[:M * N_layers].copy()
        prev_grad = g_t.storage.to_numpy().view(np.float32)[:M * N_layers].copy()
        
        # Update best
        for m in range(M):
            if losses[m] < best_losses[m]:
                best_losses[m] = losses[m]
                best_angles[m*N_layers:(m+1)*N_layers] = angles[m*N_layers:(m+1)*N_layers]
        
        # Check convergence (all starts with loss < threshold are converged)
        if iteration % 10 == 0 and np.median(best_losses) < 1e-6:
            break
    
    return best_angles.reshape(M, N_layers), best_losses


if __name__ == "__main__":
    print("GPU iRprop solver", flush=True)
    print("=" * 50, flush=True)
    
    dev = sl.create_device(type=sl.DeviceType.cuda)
    print("Loading Slang module...", flush=True)
    
    mod = sl.Module.load_from_source(dev, "irprop_step", SLANG_IRPROP_STEP)
    print("  Module loaded!", flush=True)
    
    # Test iRprop step
    M = 10
    lams = np.random.RandomState(42).random((M, N)).astype(np.float32) * np.pi - np.pi / 2
    target = LP_VIQUERAT.astype(np.float32)
    
    print(f"\nTesting {M} starts, 100 iterations...", flush=True)
    t0 = time.perf_counter()
    best_angles, best_losses = gpu_batch_irprop(dev, mod, lams, target, max_iter=100)
    t = time.perf_counter() - t0
    print(f"  Time: {t:.3f}s", flush=True)
    print(f"  Best loss: {np.min(best_losses):.2e}", flush=True)
    print(f"  Losses: {best_losses[:5]}", flush=True)
    
    # Verify against CPU
    from src.numba_solver import optimize_laminate_numba
    from src.lp_functions import compute_lp_rmse
    
    # Test single start comparison
    print("\nCPU comparison for first start:", flush=True)
    cpu_lam, cpu_loss = optimize_laminate_numba(
        lams[:1].copy(), target, n_coarse_fine=1, irprop_grad_tol=1e-3
    )
    cpu_rmse = compute_lp_rmse(cpu_lam[0], target)
    gpu_rmse = compute_lp_rmse(best_angles[0].astype(np.float32), target)
    print(f"  CPU RMSE: {cpu_rmse:.2e}", flush=True)
    print(f"  GPU RMSE: {gpu_rmse:.2e}", flush=True)
    
    # Batch benchmark
    print("\nGPU iRprop batch benchmark:", flush=True)
    for M in [100, 500, 1000, 2000, 5000]:
        lams = np.random.RandomState(42).random((M, N)).astype(np.float32) * np.pi - np.pi / 2
        t0 = time.perf_counter()
        best_a, best_l = gpu_batch_irprop(dev, mod, lams, target, max_iter=50)
        t = time.perf_counter() - t0
        print(f"  M={M:5d}: {t:.3f}s ({M/t:.0f} starts/s), best loss: {np.min(best_l):.2e}", flush=True)